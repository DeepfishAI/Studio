...
import { eventBus, getTaskContext, updateTaskStatus, getActiveTasks, BusOps } from '../infra/bus.js';
import { spawnIntern, spawnInternTeam, getActiveInterns } from '../domain/interns.js';
...
class Orchestrator {
    ...
    onHandoff({ agentId, toAgentId, taskId, content }) {
        ...
        const agent = getAgent(toAgentId);
        agent.execute(taskId, instructions).catch(err => {
    ...
    async onDispatch(msg) {
        ...
        agent.execute(taskId, content).catch(err => {
