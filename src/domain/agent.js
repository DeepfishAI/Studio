...
import { chat, chatWithTools, isLlmAvailable } from '../services/llm.js';
import { buildPlanFromLlmPolicy } from '../services/llmPolicy.js';
import { getFactsForPrompt } from './memory.js';
import { eventBus, BusOps } from '../infra/bus.js'; // <-- WIRED to the nervous system
import { queryKnowledge, checkSafety } from './knowledge/rag.js';
...
export class Agent {
    ...
    async process(input) {
        ...
        let executionPlan = [];
        let policyAllowlist = null;

        // LLM Policy (God Mode): per-agent allowlist + primary/secondary/backup
        const policyResolved = buildPlanFromLlmPolicy(this.profile.agent);
        if (policyResolved) {
            executionPlan = policyResolved.executionPlan.map(p => ({ tier: p.tier, config: p.config, backendId: p.backendId }));
            policyAllowlist = policyResolved.allowedBackends;
        }

        if (!executionPlan.length && models) {
        ...
        } else if (!executionPlan.length && legacyModel) {
    ...
    /**
     * Execute a Task (The "Brain" Loop)
     * Handles Safety, RAG, Processing, and Bus Operations
     * Formerly part of Orchestrator.js
     */
    async execute(taskId, instructions) {
        // Hydrate to access profile
        await this.hydrate();
        const agentId = this.agentId;

        // --- BRIDGE: SAFETY CHECK (INPUT) ---
        const agentProfile = this.profile?.agent;
        if (agentProfile?.safety?.enabled) {
            try {
                const isSafe = await checkSafety(instructions, 'input');
                if (!isSafe) {
                    console.warn(`[${this.name}] 🛡️ Safety Block (Input)`);
                    await BusOps.BLOCKER(agentId, taskId, "Safety violation detected in input instructions.");
                    return;
                }
            } catch (err) {
                console.warn(`[${this.name}] Safety check failed: ${err.message}`);
                // Fail open or closed? Orchestrator failed open.
            }
        }
        ...
        if (agentProfile?.knowledge?.enabled) {
            console.log(`[${this.name}] 🧠 Fetching knowledge...`);
            const collections = agentProfile.knowledge.collections || ['default'];
            try {
                const contextChunk = await queryKnowledge(instructions, collections[0]);
                if (contextChunk) {
                    knowledgeContext = `\nRELEVANT KNOWLEDGE:\n"${contextChunk}"\n`;
                    await BusOps.KNOWLEDGE(agentId, taskId, contextChunk.substring(0, 80) + "...");
                }
            } catch (err) {
                console.warn(`[${this.name}] RAG unavailable: ${err.message}`);
            }
        }
...
If blocked, end with [[BLOCKER: reason]].`;
        ...
        const hasTools = agentTools?.fileSystem || agentTools?.codeExecution || agentTools?.imageGeneration || agentTools?.webSearch || agentTools?.subagents;
        ...
        if (hasTools) {
            ...
            // ============================================
            console.log(`[${this.name}] 🔧 Has tools - using processWithTools()`);

            try {
                const result = await this.processWithTools(fullContext, {
                    maxSteps: 5,
                    forceToolUse: false
                });
                ...
                console.log(`[${this.name}] 🔧 Completed. Tools executed: ${toolResults.length}`);

                // Log tool results for debugging
                if (toolResults.length > 0) {
                    console.log(`[${this.name}] 🔧 Tool results:`);
                    toolResults.forEach((tr, i) => {
                        console.log(`  ${i + 1}. ${tr.tool}: ${tr.result?.substring(0, 100)}...`);
                    });
                }
            } catch (err) {
                console.error(`[${this.name}] 🔧 processWithTools failed:`, err.message);
            ...
            // ============================================
            console.log(`[${this.name}] Thinking (legacy mode)...`);
            response = await this.process(fullContext);
        }

        // --- BRIDGE: SAFETY CHECK (OUTPUT) ---
        if (agentProfile?.safety?.enabled) {
            try {
                const isSafe = await checkSafety(response, 'output');
                if (!isSafe) {
                    console.warn(`[${this.name}] 🛡️ Safety Block (Output)`);
